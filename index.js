...
<body class="min-h-screen bg-gray-100 flex items-center justify-center p-4" style="background-image: url('https://storage.googleapis.com/a1aa/image/89c3b8d3-c135-4ecb-844b-59110195a649.jpg'); background-size: cover; background-position: center;">
  <div class="bg-white bg-opacity-90 backdrop-blur-md rounded-lg shadow-lg max-w-md w-full p-6 text-gray-900 font-sans space-y-6">
    <h1 class="text-3xl font-extrabold text-center mb-4 text-red-600">💥PROMO💥</h1>
  ...
  <script>
    const promoCodes = {
      ...
      "PROMO20": 0.2,  // 20% discount
      "SALE30": 0.3    // 30% discount
    };
    ...
    document.getElementById('orderForm').addEventListener('submit', function(e) {
      ...
      let message = '';
      if (promoCode === "AKUBELI123") {
        message = `Halo Admin, saya ingin memesan produk dengan durasi: ${productOption.value} dan menggunakan kode promo potong harga 5.000`;
      } else if (promoCode && promoCodes[promoCode]) {
        message = `Halo Admin, saya ingin memesan produk dengan durasi: ${productOption.value} dengan kode promo ${promoCodeRaw}`;
      } else {
        message = `Halo Admin, saya ingin memesan produk dengan durasi: ${productOption.value}`;
      }
